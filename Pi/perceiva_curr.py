...
SERVER_URL = "http://192.168.111.134:4000"
...
def init_still_camera():
    """
    Initialize and warm up the still camera once at program startup.
    Stores the ready camera in the global `picam_still`.
    """
    global picam_still
    if not CAMERA_AVAILABLE:
        print("[Camera] Picamera2 not available, skipping init")
        return
    try:
        ...
        cam.start()
        time.sleep(2)  # One-time warmup
        picam_still = cam
        print("[Camera] Still camera ready")
    except Exception as e:
        print(f"[Camera] Init error: {e}")
        picam_still = None


def release_still_camera():
    """Stop and close the global still camera on shutdown."""
    global picam_still
    if picam_still:
        try:
            picam_still.stop()
            picam_still.close()
            print("[Camera] Still camera released")
        except Exception as e:
            print(f"[Camera] Release error: {e}")
        picam_still = None


def capture_image_preinitialized(output_path: str) -> bool:
    """
    Capture a still image using the already-initialized global camera.
    Much faster than capture_image() — no warmup delay.

    Args:
        output_path: Path to save the captured JPEG
    ...
    global picam_still
    if not picam_still:
        print("[Camera] Still camera not initialized, falling back to slow capture")
        return capture_image(output_path)

    # Acquire lock — blocks if another interaction is still capturing
    acquired = camera_lock.acquire(timeout=8)
    if not acquired:
        # Do NOT call release() — we don't own the lock
        print("[Camera] Camera busy (timeout), falling back to slow capture")
        return capture_image(output_path)

    # Only reached when we own the lock
    try:
        print(f"[Camera] Fast capture to {output_path}...")
        picam_still.capture_file(output_path)
        file_size = os.path.getsize(output_path) if os.path.exists(output_path) else 0
        print(f"[Camera] Captured ({file_size / 1024:.0f} KB)")
        return file_size > 0
    except Exception as e:
        print(f"[Camera] Fast capture error: {e}")
        # Camera may be in a bad state — reset it so the next interaction is safe
        print("[Camera] Resetting camera after error...")
        release_still_camera()
        init_still_camera()
        return False
    finally:
        camera_lock.release()  # Safe: only reached when acquired == True


def capture_image(output_path: str) -> bool:
    """
    Capture an image using the Raspberry Pi camera (slow path, initializes each time).
    Prefer capture_image_preinitialized() for lower latency.
...
# Global pre-initialized still camera (initialized once at startup)
picam_still = None
# Lock to prevent concurrent camera access from overlapping interactions
camera_lock = threading.Lock()
# Flag to reject new touches while an interaction is still in progress
is_processing = False
...
async def initiate_video_call():
    ...
    try:
        ...
        # End call event — set by touch sensor OR remote disconnect
        call_active = True
        end_call_event = threading.Event()

        @room.on("participant_disconnected")
        def _on_participant_disconnected(participant):
            print(f"[VideoCall] Participant left: {participant.identity} — ending call")
            end_call_event.set()

        @room.on("disconnected")
        def _on_disconnected(*args):
            print("[VideoCall] Room disconnected — ending call")
            end_call_event.set()
        ...
        # ===== Main video loop =====
        print("[VideoCall] Call active. Touch sensor or remote disconnect to end call.")
...
# Commands that need an image pre-captured
IMAGE_COMMANDS = {
    "CAPTURE_PRODUCT_IMAGE",
    "CAPTURE_ENVIRONMENT",
    "CAPTURE_MEDICAL_IMAGE",
    "CAPTURE_CURRENCY_IMAGE",
    "CAPTURE_SHELF_IMAGE",
    "CAPTURE_PRICE_IMAGE",
}
...
def process_single_interaction():
    """
    Handle a single touch-triggered interaction with parallel image capture:
    1. On touch: start image capture thread + record audio simultaneously
    2. STT → /pi_intent to get command
    3. If image needed: join thread (usually already done) → send to API
    4. If no image needed: thread finishes naturally, temp file is deleted
    ...
    # Result holder for the background capture thread
    capture_result = [False]

    def _capture_worker():
        t0 = time.time()
        capture_result[0] = capture_image_preinitialized(image_path)
        print(f"[Workflow] Background capture finished in {time.time() - t0:.2f}s")

    try:
        # ── Step 1: Start image capture + audio recording in parallel ──
        print("[Workflow] Starting parallel audio record + image capture...")
        image_thread = threading.Thread(target=_capture_worker, daemon=True)
        image_thread.start()
        ...
        # ── Step 4: If image needed, join thread (usually already done) ──
        if action_command in IMAGE_COMMANDS:
            print("[Workflow] Waiting for image capture to complete...")
        ...
        if action_command == "CAPTURE_PRODUCT_IMAGE":
            print("[Workflow] Sending image for product identification...")
            ...
            print("[Workflow] Sending image for product finding...")
            audio_response = send_image_to_find_product(image_path, transcribed_text)
            if audio_response is None:
                print("[Workflow] Product finding failed")
                return False

        elif action_command == "CAPTURE_PRICE_IMAGE":
            print("[Workflow] Sending image for price comparison...")
            ...
            # Release still camera so video call can open its own camera instance
            release_still_camera()
            try:
                ...
                # Re-initialize still camera for subsequent interactions
                init_still_camera()
            ...
            print(f"[Workflow] Command '{action_command}' not yet implemented")
            return True
        ...
        # Wait for image thread to finish (can't cancel Python threads)
        if 'image_thread' in dir() and image_thread.is_alive():
            image_thread.join(timeout=10)
        # Clean up temp files
        for path in [recording_path, image_path]:
            try:
                os.unlink(path)
            except:
                pass
...
def main():
    ...
    # Setup GPIO and pre-initialize camera (one-time warmup)
    setup_gpio()
    init_still_camera()
    ...
    try:
        ...
        while True:
            ...
            if is_processing:
                print("[Main] Busy, ignoring touch")
                continue
